...
format_count <- function(x) format(x, big.mark = ",", decimal.mark = ".", scientific = FALSE, trim = TRUE)

format_prob <- function(x) {
  paste0(format(round(x * 100, 1), nsmall = 1, big.mark = ",", decimal.mark = ".", scientific = FALSE, trim = TRUE), "%")
}

format_param <- function(x) format(x, nsmall = 2, big.mark = ",", decimal.mark = ".", scientific = FALSE, trim = TRUE)

format_prevalence <- function(x) {
  matches <- gregexpr("\\d+", x)
  extracted <- regmatches(x, matches)
  formatted <- lapply(extracted, function(vals) {
    sapply(vals, function(v) format(as.numeric(v), big.mark = ",", decimal.mark = ".", scientific = FALSE, trim = TRUE))
  })
...
wilson_ci <- function(x, n, conf_level = 0.95) {
  ...
  c(estimate = p_hat, ci_lower = max(0, center - margin), ci_upper = min(1, center + margin))
}
...
build_plot <- function(row, prior_mendelian_n, prior_total_n) {
  ...
  plot_df <- data.frame(
    ...
    monogenic_label = c(format_prob(prior_monogenic), format_prob(post_monogenic)),
    polygenic_label = c(format_prob(prior_polygenic), format_prob(post_polygenic)),
  ...
  plot_df$monogenic_hover <- ifelse(
    ...
    paste0("<b>", plot_df$scenario, "</b><br>Monogenic: ", plot_df$monogenic_label, "<br>95% CI: ",
           plot_df$ci_low_label, " \u2013 ", plot_df$ci_high_label, "<br>n = ",
  ...
  plot_ly(plot_df) %>%
    add_trace(x = ~monogenic, y = ~scenario, name = "Monogenic", type = "bar", orientation = "h",
      marker = list(color = "#ed2024", line = list(color = "white", width = 1)),
      text = ~monogenic_label, textposition = "inside", insidetextanchor = "middle",
      textfont = list(size = 11, color = "white"), hovertemplate = ~monogenic_hover) %>%
    add_trace(x = ~polygenic, y = ~scenario, name = "Polygenic", type = "bar", orientation = "h",
      marker = list(color = "#5cbcd6", line = list(color = "white", width = 1)),
      text = ~polygenic_label, textposition = "inside", insidetextanchor = "middle",
      textfont = list(size = 11, color = "white"), hovertemplate = ~polygenic_hover) %>%
    layout(
      barmode = "stack", bargap = 0.25, font = list(size = 13),
      uniformtext = list(minsize = 9, mode = "show"),
      hoverlabel = list(font = list(size = 12, color = "white")),
      xaxis = list(title = "", tickformat = ".0%", range = c(0, 1), showgrid = TRUE,
                   gridcolor = "#e9ecef", zeroline = FALSE, fixedrange = TRUE),
      yaxis = list(title = "", automargin = TRUE, fixedrange = TRUE),
      legend = list(orientation = "h", xanchor = "center", x = 0.5, yanchor = "bottom", y = 1.01,
                    font = list(size = 12), traceorder = "normal"),
      margin = list(l = 8, r = 8, t = 8, b = 8), dragmode = FALSE
    ) %>%
    config(displayModeBar = FALSE, staticPlot = FALSE, scrollZoom = FALSE, doubleClick = FALSE, showTips = FALSE)
}

build_plot_var_compare <- function(row_main_ref, row_var, prior_mendelian_n, prior_total_n) {
  prior_monogenic <- prior_mendelian_n / prior_total_n
  prior_polygenic <- 1 - prior_monogenic
  main_monogenic <- row_main_ref$n_mendelian / row_main_ref$n
  main_polygenic <- row_main_ref$n_non_mendelian / row_main_ref$n
  var_monogenic <- row_var$n_mendelian / row_var$n
  var_polygenic <- row_var$n_non_mendelian / row_var$n

  plot_df <- data.frame(
    scenario = factor(
      c("Prior probability<br>(selected params)", "Family history<br>(selected params)", "Family history<br>(main simulation)"),
      levels = c("Family history<br>(main simulation)", "Family history<br>(selected params)", "Prior probability<br>(selected params)")
    ),
    monogenic = c(prior_monogenic, var_monogenic, main_monogenic),
    polygenic = c(prior_polygenic, var_polygenic, main_polygenic),
    monogenic_n = c(prior_mendelian_n, row_var$n_mendelian, row_main_ref$n_mendelian),
    polygenic_n = c(prior_total_n - prior_mendelian_n, row_var$n_non_mendelian, row_main_ref$n_non_mendelian),
    total_n = c(prior_total_n, row_var$n, row_main_ref$n),
    monogenic_label = c(format_prob(prior_monogenic), format_prob(var_monogenic), format_prob(main_monogenic)),
    polygenic_label = c(format_prob(prior_polygenic), format_prob(var_polygenic), format_prob(main_polygenic)),
  ...
  plot_df$monogenic_hover <- ifelse(
    plot_df$scenario == "Prior probability<br>(selected params)",
    paste0("<b>", plot_df$scenario, "</b><br>Monogenic: ", plot_df$monogenic_label, "<br>n = ",
           format_count(plot_df$monogenic_n), " of ", format_count(plot_df$total_n), "<extra></extra>"),
    paste0("<b>", plot_df$scenario, "</b><br>Monogenic: ", plot_df$monogenic_label, "<br>95% CI: ",
           plot_df$ci_low_label, " \u2013 ", plot_df$ci_high_label, "<br>n = ",
...
ui <- page_fluid(
  ...
  tags$head(
    tags$style(HTML("
      .app-shell { max-width: 1600px; margin: 0 auto; padding-top: 10px; padding-bottom: 20px; }
      .app-header { display: flex; align-items: center; justify-content: flex-start; gap: 16px; flex-wrap: wrap; margin-bottom: 18px; padding: 8px 4px 2px 4px; }
      .app-header-logo { height: 82px; width: auto; object-fit: contain; flex-shrink: 0; }
      .app-header-text { min-width: 280px; }
      .app-title { font-size: 24px; font-weight: 600; line-height: 1.2; }
      .app-subtitle { font-size: 18px; font-weight: 400; line-height: 1.2; }
      .app-fullname { font-size: 15px; color: #555; margin-top: 4px; }
      .card-header { font-size: 15px !important; font-weight: 600; }
      .card-body, .card-body p, .card-body div, .card-body span,
      .sidebar, .sidebar .form-label, .sidebar .selectize-input, .sidebar .selectize-dropdown,
      .sidebar .form-select, .sidebar .control-label, .sidebar label,
      .table, .table th, .table td, .shiny-input-container, .shiny-input-container label,
      .form-control, .form-select { font-size: 13px !important; line-height: 1.5; }
      .sidebar .form-label, .sidebar label, .sidebar .control-label, .table th { font-weight: 600; }
      ...
      nav_panel("Main simulation",
        layout_sidebar(
          ...
          layout_columns(col_widths = c(8, 4),
            card(full_screen = FALSE, card_header("How to count relatives"),
              div(style = "padding: 4px; height: 100%; overflow: hidden;",
                tags$img(src = "APECS_relative_count.svg",
...
server <- function(input, output, session) {
  ...
  selected_row_main <- reactive({
    ...
    validate(need(nrow(df) > 0, "No matching pedigree pattern found in the lookup table."))
    ...
    agg <- df %>%
      summarise(
        ...
        n = sum(n), n_mendelian = sum(n_mendelian), n_non_mendelian = sum(n_non_mendelian)
      )
  ...
  selected_row_var <- reactive({
    ...
    validate(need(nrow(df) > 0, "No matching pedigree pattern found for this family history and parameter combination."))
  ...
  selected_row_var_main_ref <- reactive({
    df <- if (input$mode_var == "ALS only") als_grid else alsftd_grid
  ...
  output$prob_bar_var <- renderPlotly({
    ...
    build_plot_var_compare(
      ...
      row_var = row_var,
      prior_mendelian_n = prior$prior_mendelian_n[[1]],
      prior_total_n = prior$prior_total_n[[1]]
    )
  })
